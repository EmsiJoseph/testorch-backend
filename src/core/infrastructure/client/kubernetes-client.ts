...
export class KubernetesClient implements IKubernetesClient {
    ...
    async createRoleBinding(namespace: string, roleBindingManifest: k8s.V1RoleBinding): Promise<void> {
        try {
    ...
    async deploymentExists(deploymentName: string, namespace: string): Promise<boolean> {
        try {
