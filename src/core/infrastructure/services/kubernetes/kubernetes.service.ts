...
import { existsSync, promises as fsPromises } from 'fs'; // Import existsSync and fs.promises from fs
import fs from 'fs';
...
export class KubernetesService implements OnModuleInit, IKubernetesService {
  ...
  public async waitForPodReady(serviceName: string, namespace: string, timeout: number = 300000): Promise<void> {
    ...
    throw new Error(`Pod ${serviceName} in namespace ${namespace} is not ready after ${timeout / 1000} seconds`);
  }
  ...
  public async getPodName(serviceName: string, namespace: string): Promise<string> {
    try {
      ...
      const pod = body.items.find((pod) => pod.status?.phase === 'Running');
      if (!pod) {
        throw new Error(`Pod for service ${serviceName} not found in namespace ${namespace}`);
      }
  ...
  public async deleteAndRedeployJmeterMaster(): Promise<void> {
    ...
    try {
      ...
      await this.kubernetesClient.k8sAppsApi.deleteNamespacedDeployment(deploymentName, namespace);
      this.logger.log(`Deleted deployment ${deploymentName} in namespace ${namespace}`);

      // Delete the service
      await this.kubernetesClient.k8sCoreApi.deleteNamespacedService(serviceName, namespace);
      this.logger.log(`Deleted service ${serviceName} in namespace ${namespace}`);
      ...
      const errorMessage = this.getErrorMessage(error);
      this.logger.error(`Failed to delete and redeploy ${deploymentName}: ${errorMessage}`);
      throw new Error(`Failed to delete and redeploy ${deploymentName}: ${errorMessage}`);
    }
  }

  public async getPodStatus(podName: string, namespace: string): Promise<string> {
    try {
      const { body } = await this.kubernetesClient.k8sCoreApi.readNamespacedPodStatus(podName, namespace);
      return body.status?.phase || 'Unknown';
    } catch (error) {
      const errorMessage = this.getErrorMessage(error);
      this.logger.error(`Failed to get status for pod ${podName}: ${errorMessage}`);
      throw new Error(`Failed to get status for pod ${podName}: ${errorMessage}`);
    }
  }
  ...
  public async editDeploymentYaml(
    ...
    try {
      ...
      const formattedFilePath = path.join(mountPath, path.basename(filePath)).replace(/\\/g, '/'); // Ensure correct path formatting for Windows
      yamlContent = yamlContent.replace(/\/path\/to\/your\/uploads\/directory/g, mountPath);
      yamlContent = yamlContent.replace(/\/uploads\/blazeDemoTest.jmx/g, formattedFilePath);
      yamlContent = yamlContent.replace(/value: "\/uploads\/blazeDemoTest.jmx"/g, `value: "${formattedFilePath}"`);
  ...
  public async getPodsByLabel(label: string, namespace: string): Promise<any[]> {
    try {
      const { body } = await this.kubernetesClient.k8sCoreApi.listNamespacedPod(
        namespace,
        undefined,
      ...
      const errorMessage = this.getErrorMessage(error);
      this.logger.error(`Failed to get pods by label ${label}: ${errorMessage}`);
      throw new Error(`Failed to get pods by label ${label}: ${errorMessage}`);
    }
  }

  public async deleteDeployment(deploymentName: string, namespace: string): Promise<void> {
    try {
      await this.kubernetesClient.k8sAppsApi.deleteNamespacedDeployment(deploymentName, namespace);
      this.logger.log(`Deleted deployment ${deploymentName} in namespace ${namespace}`);
    } catch (error) {
      const errorMessage = this.getErrorMessage(error);
      this.logger.error(`Failed to delete deployment ${deploymentName} in namespace ${namespace}: ${errorMessage}`);
      throw new Error(`Failed to delete deployment ${deploymentName} in namespace ${namespace}: ${errorMessage}`);
    }
  }
